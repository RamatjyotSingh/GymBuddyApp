package comp3350.gymbuddy.logic;

public class LogicConfig {
    public final static String invalidNameExceptionMessage = "Name must be provided.";
    public final static String invalidInputExceptionMessage = "Exercises must be provided.";
    public final static String integerFormatExceptionMessage = "Must be a valid integer.";
    public final static String invalidNonzeroValueMessage = "Must be at least 1.";
    public final static String doubleFormatExceptionMessage = "Must be a valid number.";
    public final static String invalidNonnegativeValueMessage = "Must be at least 0.";
}
